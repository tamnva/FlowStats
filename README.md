...
- This is an R package (with Graphical User Interface - GUI) for streamflow visualization and statistical analysis (real-time and forecast up to 5 day in advance) using Long-Short-Term Memory (LSTM) rainfall-runoff model and meterological data ([daily precipitation, temperature (min & max), and relative humidity](https://opendata.dwd.de/climate_environment/CDC/grids_germany/daily/hyras_de/)) from the German Weather Service (DWD).
...
- The simulated streamflow data (01.1980 - 5.2025) are used as default for statistical streamflow analysis (this data can be updated to get real time and forecast streamflow data).
