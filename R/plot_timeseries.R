...
#'
#' @param plot_type character, can be one of the following options 'Daily',
#' 'Daily (by year)' for minimum, 'Daily cumsum (by year)'
#'
...
#' plt_timeseries <- plot_timeseries(Q_input, gaugeid, plot_type, log_y = FALSE)
#' }
#' @export
#'
...
plot_timeseries <- function(Q_input, gaugeid, plot_type, log_y){
  ...
  if (plot_type == "Daily (by year)"){
    Q_daily_stat <- Q_gauge_id %>%
      ...
      dplyr::summarise(
        Q_min = min(q_mm_day),
        `Q_10%` = quantile(q_mm_day, c(0.05)),
        `Q_25%` = quantile(q_mm_day, c(0.25)),
        `Q_75%` = quantile(q_mm_day, c(0.75)),
        `Q_90%` = quantile(q_mm_day, c(0.95)),
        Q_max = max(q_mm_day),
        .groups = 'drop'
      )
  } else if(plot_type == "Daily"){
    plt <- ggplot2::ggplot(tail(Q_gauge_id, 3650), ggplot2::aes(x = date, y = q_mm_day)) +
      ggplot2::geom_line(color = "#0072B2", linewidth = 0.3, alpha = 0.8) +
      ggplot2::geom_point(color = "#0072B2", alpha = 0.8, size = 0.5) +
      ggplot2::labs(y = "Q (mm/day)", x = " ") +
      ggplot2::theme_bw() +
    ...
    Q_daily_stat_cumsum <- Q_gauge_id %>%
      dplyr::mutate(year = lubridate::year(date),
                    ...
                    day = lubridate::day(date)) %>%
      dplyr::group_by(year) %>%
      dplyr::mutate(q_mm_day = cumsum(q_mm_day))  %>%
      dplyr::group_by(month, day) %>%
      dplyr::summarise(
        Q_min = min(q_mm_day),
        `Q_10%` = quantile(q_mm_day, c(0.05)),
    ...
    plt <- ggplot2::ggplot(Q_daily_stat, ggplot2::aes(x = date)) +
      ...
      ggplot2::geom_line(
        data = Q_gauge_id %>%
          ...
          dplyr::rename(`Q_current_year` = q_mm_day),
        ggplot2::aes(x = date, y = `Q_current_year`), color = "#0072B2") +
      ggplot2::labs(y = "Q (mm/day)", x = " ") +
      ggplot2::theme_bw() +
      ggplot2::theme(axis.title = ggplot2::element_text(size = 9))
    ...
    plt <- ggplot2::ggplot(Q_daily_stat_cumsum, ggplot2::aes(x = date)) +
      ...
      ggplot2::geom_line(
        data = Q_gauge_id %>%
          dplyr::filter(lubridate::year(date) == current_year) %>%
          dplyr::rename(`Q_current_year` = q_mm_day),
        ggplot2::aes(x = date, y = cumsum(`Q_current_year`)), color = "#0072B2") +
      ...
      ggplot2::labs(y = "Q (mm/day)", x = " ", fill = "Color legend") +
      ggplot2::theme_bw() +
